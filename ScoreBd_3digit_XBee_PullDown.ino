...
void setup() {
  ...
  pinMode(btnUP, INPUT);  //dw no resistor required thanks to internal pull-up
...
void loop() {
  ...
  if (digitalRead(btnRESET) == HIGH && isPressed2 == false)  //button is pressed AND this is the first digitalRead() that the button is pressed
  ...
  if (digitalRead(btnUP) == HIGH && isPressed == false)  //button is pressed AND this is the first digitalRead() that the button is pressed
  {
  ...
  if (digitalRead(btnDOWN) == HIGH && isPressed == false)  //button is pressed AND this is the first digitalRead() that the button is pressed
  {
    isPressed = true;  //set to true, so this code will not run again until button released
...
void do7SegStatement() {
  ...
  delay(1500);  // delay between button presses (sort of a tactile-ish adjustment)
}
