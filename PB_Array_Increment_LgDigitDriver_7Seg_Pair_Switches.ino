// dw PB Array Increment Pair of Digits to Display  00 to 99 with
// dw % Remainder Operation; Up Button; RESET Button;
...
void loop() {
  ...
  if (digitalRead(btnUP) == LOW && isPressed == false)  //button is pressed AND this is the first digitalRead() that the button is pressed
  {
    ...
    // int m = (k/100) % 10;  // gets hundreds value

    // int n = (k/1000) % 10;  // gets thousands value



    if (k > 99) {  //  limits k to 99 for two digit display; should be 999 for three digit display
      i = 0;
      j = 0;
      k = 0;
    }
